...
public class Bit {
        ...
        for (long froms = Squares; froms != 0; froms = rest(froms)) {
            ...
            for (int dir = 0; dir < Square.dir_size; dir++) {
                ...
                for (int to = from + inc; Square.is_valid(to); to += inc) {
                    ...
                    p_between[from][to] = ray;
                    p_inc[from][to] = (byte) inc;

                    ray = set(ray, to);
                }

                if (dir < 4) p_king_moves[from] |= ray;
                p_king_captures[from] |= ray;
            }
        }
    }
    ...
    static boolean is_disjoint(long b0, long b1) {
        return (b0 & b1) == 0;
    }
    ...
    public static int first(long b) {
        assert b != 0;
        return Long.numberOfTrailingZeros(b);
    }

    public static long rest(long b) {
        assert b != 0;
        return b & (b - 1);
    }
